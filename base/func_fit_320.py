...
def parse_args(check=True):
    ...
    parser.add_argument("--binary_file_path", type=str, default="rectifymap.data",
                        help="The rescale factor for x in the data file.")
    ...
    with open(filename,'r') as myFile:
        ...
        for line in lines:
            ...
            if (1 == n_lines):
                ...
                n_fields = len(var_lst)
                try:
                    max_counts = int(var_lst[col_y])
                except ValueError:
                    var_lst[col_y] = '0'
                    max_counts = len(lines) - 1
                # Continue for the first line
                distances = [float(var_lst[idx]) for idx in range(len(var_lst)) if idx != col_y]
                continue
            
            # Check the number of the fields in current line.
            n_fieldz = len(var_lst)
            if n_fieldz != n_fields:
                raise Exception("Fields count does NOT match on line: %d"%(n_lines))
            if n_lines > max_counts:
                print('Warning: Max row exceeded.....')
            # Parse the record
            value_ts = [[float(item)] for item in var_lst]            
            value_arr.append(value_ts)
    # In the end
    values_split = list(np.concatenate(value_arr, 1))
    y_vecx = list(values_split[col_y])
...
def main():
    ...
    # For version 1, b_input_merge should not be true
    if b_input_merge == True:
        raise ValueError('b_input_merge must be false for version 1')
    ...
    for filename in lst_input_path:
        # Tempeture data file format v0
        # x, y, distancx = load_data_temperature(filename, splitchar, col_x, col_y, rescale_f)
        # Tempeture data file format v1
        x_vecs, y_vecx, dists_vec = load_data_temperature_with_multiple_distance_columns(filename, splitchar, col_y, rescale_f)
...
if __name__ == '__main__':
    ...
    main()
    # filename = 'G:\\demo.csv'
    # x_vecs, y_vecx, dists_vec = load_data_temperature_with_multiple_distance_columns(filename, splitchar = ',', col_y = 0, rescale = 1.0)
    # filename = 'G:\\datav0.csv'
    # x, y, distancx = load_data_temperature(filename, splitchar = ',', col_x=1, col_y=0, rescale=1.0)
